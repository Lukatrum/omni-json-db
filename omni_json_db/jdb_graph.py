...
from .utils import JKeyError, JValueError, KF_HIDDEN, KF_UNLOCK
...
class GraphDb(JDb):
    ...
    def reindex(self) -> Dict[str,int]:
        ...
        with self.open(read_only=False) as fp:
            ...
            for node_id, entries in new_adj.items():
                f_write(fp, f'X:{node_id}:', entries, overwrite=True, max_wsize=0, key_flags=KF_UNLOCK)

            f_write(fp, self.N_NODES, n_nodes, key_flags=KF_UNLOCK)
            f_write(fp, self.N_EDGES, n_edges, key_flags=KF_UNLOCK)
            f_write(fp, self.N_DIRECTED, n_di_edges, key_flags=KF_UNLOCK)
    ...
    def f_add_node(self, fp_dict:Dict[int,IO], node_id:str, **properties) -> bool:
        ...
        if node_key not in self.io.key_table:
            if f_write(fp_dict, node_key, properties, key_flags=KF_UNLOCK):
                n_nid = self.N_NODES
                f_write(fp_dict, n_nid, max(f_read(fp_dict, n_nid, 0),0)+1, key_flags=KF_HIDDEN|KF_UNLOCK)
                return True
            return False
        ...
        if isinstance(old_props, dict):
            ...
            if new_props != old_props:
                return f_write(fp_dict, node_key, new_props, key_flags=KF_UNLOCK)
            else:
                return False

        return f_write(fp_dict, node_key, properties, key_flags=KF_UNLOCK)
    ...
    def f_remove_node(self, fp_dict:Dict[int,IO], node_id:str) -> Dict[str,Any]:
            ...
            for adj_id in self.f_get_adj(fp_dict, node_id):
                ...
                if adj_row >= 0 \
                    ...
                    if not new_adj:
                        ...
                        f_write(fp_dict, adj_key, new_adj, overwrite=True, max_wsize=0, key_flags=KF_UNLOCK)
        ...
        if matched_keys:
            ...
            for row_id,key in matched_keys:
                val = f_delete(fp_dict, key, row=row_id, key_flags=KF_UNLOCK)
                ret[key] = val
                if key.startswith('N:'):
                    n_nodes -= 1
            ...
            f_write(fp_dict, n_nid, max(0, n_nodes), key_flags=KF_UNLOCK)
            f_write(fp_dict, n_eid, max(0, n_edges), key_flags=KF_UNLOCK)
            f_write(fp_dict, n_did, max(0, n_di_edges), key_flags=KF_UNLOCK)
    ...
    def f_add_edge(self, fp_dict:Dict[int,IO], u:str, v:str, directed:bool=True, **properties) -> bool:
        ...
        if edge_key not in key_table:
            ...
            if u_key not in key_table:
                f_write(fp_dict, u_key, {}, overwrite=True, max_wsize=0, key_flags=KF_UNLOCK)
                n_nodes += 1

            v_key = f'N:{v}:'
            if v_key not in key_table:
                f_write(fp_dict, v_key, {}, overwrite=True, max_wsize=0, key_flags=KF_UNLOCK)
                n_nodes += 1
            ...
            if xu_val not in adj:
                adj.append(xu_val)
                f_write(fp_dict, xu_key, adj, overwrite=True, max_wsize=0, key_flags=KF_HIDDEN|KF_UNLOCK)
            ...
            if xv_val not in adj:
                adj.append(xv_val)
                f_write(fp_dict, xv_key, adj, overwrite=True, max_wsize=0, key_flags=KF_HIDDEN|KF_UNLOCK)

            f_write(fp_dict, n_nid, max(0, n_nodes), key_flags=KF_HIDDEN|KF_UNLOCK)
            if f_write(fp_dict, edge_key, properties, key_flags=KF_UNLOCK):
                n_eid = self.N_EDGES
                f_write(fp_dict, n_eid, max(f_read(fp_dict, n_eid, 0), 0) + 1, key_flags=KF_HIDDEN|KF_UNLOCK)
                if directed:
                    n_did = self.N_DIRECTED
                    f_write(fp_dict, n_did, max(f_read(fp_dict, n_did, 0), 0) + 1, key_flags=KF_HIDDEN|KF_UNLOCK)
        ...
        if not isinstance(old_props, dict): # pragma: no cover
            return self.f_write(fp_dict, edge_key, properties, key_flags=KF_UNLOCK)
    ...
    def f_remove_edge(self, fp_dict:Dict[int,IO], u:str, v:str, directed:bool=True) -> Dict[str,Any]:
        ...
        if edge_key in key_table:
            ...
            f_read = self.f_read
            _io, fp_dict, _key_fp, _sync_chg = self.f_get_write_fp(fp_dict)
            props = self.f_delete(fp_dict, edge_key, key_flags=KF_UNLOCK)
            ret[edge_key] = props
            n_eid = self.N_EDGES
            f_write(fp_dict, n_eid, max(0, f_read(fp_dict, n_eid, 0) - 1), key_flags=KF_UNLOCK)
            if directed:
                n_did = self.N_DIRECTED
                f_write(fp_dict, n_did, max(0, f_read(fp_dict, n_did, 0) - 1), key_flags=KF_UNLOCK)
            ...
            if xu_val in adj:
                adj.remove(xu_val)
                f_write(fp_dict, xu_key, adj, overwrite=True, max_wsize=0, key_flags=KF_UNLOCK)
            ...
            if xv_val in adj:
                adj.remove(xv_val)
                f_write(fp_dict, xv_key, adj, overwrite=True, max_wsize=0, key_flags=KF_UNLOCK)
